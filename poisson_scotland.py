...
with open('Szkocja_20201225.csv', newline = '') as csvfile:
    ...
    next(_reader)
    for row in _reader:
        predictors.append((row[0], row[1], (int(row[2]), int(row[3])), int(row[6])))

counts = [0]*6
counts_25 = [0,0]
counts_sum = [0,0]
matches = [[0,0] for _ in range(20)]
for pred in predictors:
    ...
    if _scores_sum[0][1]+_scores_sum[1][1] > 0.4:
        ...
        if sum(pred[2]) in (_scores_sum[0][0], _scores_sum[1][0]):
            counts_sum[0] += 1
            matches[pred[3] - 1][0] += 1
        else:
            matches[pred[3] - 1][1] += 1
            counts_sum[1] += 1
