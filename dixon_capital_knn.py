...
def find_prob(key, p, delta=0.025, alpha=0.01):
    pos_ct, ct = 0, 0
    ...
    if '_' in file: #and '2020' in file:
        ...
        for i in range(ind_start, df.shape[0]-9, 9):
            ...
            for j in range(9):
                ...
                if conf_int[0]*df_out.iloc[j]['Avg25u'] > 1.02: #and df_out.iloc[j]['Avg25u'] > 1.3:
                    bets_to_do.append((j, '2.5 under', conf_int[0], df_out.iloc[j]['Avg25u']))

                if (1-conf_int[1]) * df_out.iloc[j]['Avg25o'] > 1.02: #and df_out.iloc[j]['Avg25o'] > 1.3:
            ...
            for k in range(len(bets_to_do)):
                ...
                if f > 0:
                    ...
                    if win: capital += (f*init_capital*r)
                    ct_bets += 1
